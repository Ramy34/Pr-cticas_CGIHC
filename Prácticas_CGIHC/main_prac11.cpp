...
typedef struct _frame
{
	...
	float rotRodDer; //Movimiento pierna derecha
	float rotInc;
	float giroMonito; //Movimiento del personaje
	float giroMonitoInc;
	float movBrazo; //Movimiento del brazo izquierdo
	float movBrazoInc;
	float movBrazoDer; //Movimiento del brazo derecho
	float movBrazoDerInc;
	float movRodIzq; //Moviemiento de la pierna izquierda
	float movRodIzqInc;
	float movCabeza; //Movimiento de la cabeza
	float movCabezaInc;
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].movRodIzq = movRodIzq;
...
void resetElements(void)
{
	...
	movRodIzq = KeyFrame[0].movRodIzq;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].movRodIzqInc = (KeyFrame[playIndex + 1].movRodIzq - KeyFrame[playIndex].movRodIzq) / i_max_steps;
...
void animate(void)
{
	if (play)
	{
		...
		else
		{
			...
			movRodIzq += KeyFrame[playIndex].movRodIzqInc;
	...
	Model brazoDer, Model brazoIzq, Model cabeza, Model piso)
{
	...
	//Pierna Izq
	model = glm::translate(tmp, glm::vec3(0.5f, 0.0f, -0.1f));
	model = glm::rotate(model, glm::radians(movRodIzq), glm::vec3(1.0f, 0.0f, 0.0f));
	shader.setMat4("model", model);
